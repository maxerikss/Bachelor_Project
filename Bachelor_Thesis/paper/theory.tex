...
\end{equation}
where $\bar{\lambda}$ represents the strength of the measurement, $z$ is a continuous outcome of the measurement, and $\hat{A}$ is the measured observable \cite{Annby-Andersson:2024}. We note that the outcome of such a measurement is described by
\begin{equation}
    \expval{\hat{A}} = \frac{\hat{K}(z) \dmatrix \hat{K}^\dagger(z)}{p(z)},
\end{equation}
